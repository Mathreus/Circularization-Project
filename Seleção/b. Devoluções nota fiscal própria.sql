SELECT
    ...
    SUM(CASE WHEN TIPO_OPERACAO = 'VENDA' THEN QUANTIDADE ELSE 0 END) AS QTD_VENDAS,
    SUM(CASE WHEN TIPO_OPERACAO = 'VENDA' THEN VALOR ELSE 0 END) AS VALOR_VENDAS,
    
    -- Métricas para devoluções
    SUM(CASE WHEN TIPO_OPERACAO = 'DEVOLUCAO' THEN QUANTIDADE ELSE 0 END) AS QTD_DEVOLUCOES,
    SUM(CASE WHEN TIPO_OPERACAO = 'DEVOLUCAO' THEN VALOR ELSE 0 END) AS VALOR_DEVOLUCOES,
    
    -- Cálculo 1: Devoluções por GRUPO (soma de todas as devoluções do grupo)
    SUM(CASE WHEN TIPO_OPERACAO = 'DEVOLUCAO' THEN QUANTIDADE ELSE 0 END) OVER (PARTITION BY GRUPO) AS DEVOLUCOES_POR_GRUPO,
    
    -- Cálculo 2: Percentual de Devolução por GRUPO
    CASE 
        WHEN SUM(CASE WHEN TIPO_OPERACAO = 'VENDA' THEN QUANTIDADE ELSE 0 END) OVER (PARTITION BY GRUPO) > 0 
        THEN ROUND(
            (SUM(CASE WHEN TIPO_OPERACAO = 'DEVOLUCAO' THEN QUANTIDADE ELSE 0 END) OVER (PARTITION BY GRUPO) * 100.0) / 
            SUM(CASE WHEN TIPO_OPERACAO = 'VENDA' THEN QUANTIDADE ELSE 0 END) OVER (PARTITION BY GRUPO)
        , 2)
        ELSE 0 
    END AS PERCENTUAL_DEVOLUCAO_GRUPO,
    
    -- Cálculo 3: Percentual de Devolução por ESTABELECIMENTO
    CASE 
        WHEN SUM(CASE WHEN TIPO_OPERACAO = 'VENDA' THEN QUANTIDADE ELSE 0 END) > 0 
        THEN ROUND(
            (SUM(CASE WHEN TIPO_OPERACAO = 'DEVOLUCAO' THEN QUANTIDADE ELSE 0 END) * 100.0) / 
            SUM(CASE WHEN TIPO_OPERACAO = 'VENDA' THEN QUANTIDADE ELSE 0 END)
        , 2)
        ELSE 0 
    END AS PERCENTUAL_DEVOLUCAO_ESTABELECIMENTO,

    -- Cálculo 4: Proporção deste estabelecimento nas devoluções do grupo
    CASE 
        WHEN SUM(CASE WHEN TIPO_OPERACAO = 'DEVOLUCAO' THEN QUANTIDADE ELSE 0 END) OVER (PARTITION BY GRUPO) > 0 
        THEN ROUND(
            (SUM(CASE WHEN TIPO_OPERACAO = 'DEVOLUCAO' THEN QUANTIDADE ELSE 0 END) * 100.0) / 
            SUM(CASE WHEN TIPO_OPERACAO = 'DEVOLUCAO' THEN QUANTIDADE ELSE 0 END) OVER (PARTITION BY GRUPO)
        , 2)
        ELSE 0 
    END AS PARTICIPACAO_DEVOLUCAO_NO_GRUPO
...
ORDER BY GRUPO, PERCENTUAL_DEVOLUCAO_ESTABELECIMENTO DESC;
