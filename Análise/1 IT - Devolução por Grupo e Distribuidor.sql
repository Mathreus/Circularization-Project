SELECT
    ...
    -- Métricas de Vendas (CFOPs de venda)
    SUM(CASE WHEN CFOP IN ('5.101', '5.102', '5.103', '5.104', '5.105', '5.106', '5.107', '5.108', '5.109', 
                         ...
                         '6.112', '6.113', '6.114', '6.115', '6.116') THEN QUANTIDADE ELSE 0 END) AS QTD_VENDAS,
    ...
    SUM(CASE WHEN CFOP IN ('5.101', '5.102', '5.103', '5.104', '5.105', '5.106', '5.107', '5.108', '5.109', 
                         ...
                         '5.403', '5.404', '5.405', '5.501', '5.502', '5.503', '5.504', '6.101', '6.102',
                         '6.103', '6.104', '6.105', '6.106', '6.107', '6.108', '6.109', '6.110', '6.111',
                         '6.112', '6.113', '6.114', '6.115', '6.116') THEN VALOR ELSE 0 END) AS VALOR_VENDAS,
    
    -- Métricas de Devoluções (CFOPs de devolução)
    SUM(CASE WHEN CFOP IN ('1.201', '1.202', '1.203', '1.204', '1.410', '1.411', '1.553', '1.660', '1.661', '1.662',
                         '2.201', '2.202', '2.203', '2.204', '2.410', '2.411', '2.553', '2.660', '2.661', '2.662',
                         '3.201', '3.202', '3.211', '3.553') THEN ABS(QUANTIDADE) ELSE 0 END) AS QTD_DEVOLUCOES,
    
    SUM(CASE WHEN CFOP IN ('1.201', '1.202', '1.203', '1.204', '1.410', '1.411', '1.553', '1.660', '1.661', '1.662',
                         '2.201', '2.202', '2.203', '2.204', '2.410', '2.411', '2.553', '2.660', '2.661', '2.662',
                         '3.201', '3.202', '3.211', '3.553') THEN ABS(VALOR) ELSE 0 END) AS VALOR_DEVOLUCOES,
    
    -- Cálculo do Índice de Devolução
    CASE 
        WHEN SUM(CASE WHEN CFOP IN ('5.101', '5.102', '5.103', '5.104', '5.105', '5.106', '5.107', '5.108', '5.109', 
                                  '5.110', '5.111', '5.112', '5.113', '5.114', '5.115', '5.116', '5.201', '5.202',
                                  '5.203', '5.204', '5.205', '5.206', '5.207', '5.208', '5.209', '5.401', '5.402',
                                  '5.403', '5.404', '5.405', '5.501', '5.502', '5.503', '5.504', '6.101', '6.102',
                                  '6.103', '6.104', '6.105', '6.106', '6.107', '6.108', '6.109', '6.110', '6.111',
                                  '6.112', '6.113', '6.114', '6.115', '6.116') THEN QUANTIDADE ELSE 0 END) > 0 
        THEN ROUND(
            (SUM(CASE WHEN CFOP IN ('1.201', '1.202', '1.203', '1.204', '1.410', '1.411', '1.553', '1.660', '1.661', '1.662',
                                  '2.201', '2.202', '2.203', '2.204', '2.410', '2.411', '2.553', '2.660', '2.661', '2.662',
                                  '3.201', '3.202', '3.211', '3.553') THEN ABS(QUANTIDADE) ELSE 0 END) * 100.0) / 
            SUM(CASE WHEN CFOP IN ('5.101', '5.102', '5.103', '5.104', '5.105', '5.106', '5.107', '5.108', '5.109', 
                                  '5.110', '5.111', '5.112', '5.113', '5.114', '5.115', '5.116', '5.201', '5.202',
                                  '5.203', '5.204', '5.205', '5.206', '5.207', '5.208', '5.209', '5.401', '5.402',
                                  '5.403', '5.404', '5.405', '5.501', '5.502', '5.503', '5.504', '6.101', '6.102',
                                  '6.103', '6.104', '6.105', '6.106', '6.107', '6.108', '6.109', '6.110', '6.111',
                                  '6.112', '6.113', '6.114', '6.115', '6.116') THEN QUANTIDADE ELSE 0 END)
        , 2)
        ELSE 0 
    END AS INDICE_DEVOLUCAO_PERCENTUAL,

    -- Total geral (vendas + devoluções em valor absoluto)
    SUM(QUANTIDADE) AS QTD_TOTAL,
    SUM(VALOR) AS VALOR_TOTAL,

    -- Contagem de notas fiscais
    COUNT(DISTINCT NUM_NOTA_FISCAL) AS TOTAL_NOTAS_FISCAIS
...
GROUP BY 
    ...
    GRUPO
ORDER BY 
    INDICE_DEVOLUCAO_PERCENTUAL DESC,
    VALOR_VENDAS DESC;
